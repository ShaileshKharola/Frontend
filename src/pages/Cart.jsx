      ...
      for (const sizeKey in cartItems[itemId]) {
        if (cartItems[itemId][sizeKey] > 0) {
          tempData.push({
            _id: itemId,
            size: sizeKey,
            quantity: cartItems[itemId][sizeKey],
          });
        }
      }
    }
                ...
                <div className='flex items-start gap-6'>
                  ...
                  <div>
                    ...
                    <div className='flex items-center gap-5 mt-2'>
                      <p>${productData.currency}{productData.price}</p>
                      <p className='px-2 sm:px-3 sm:py-1 border bg-slate-50'>Size: {item.size}</p>
                    </div>
                  </div>
                </div>
                <input className='border max-w-10 sm:max-w-20 px-1 sm:px-1 py-1' type="number" min={1} defaultValue={item.quantity} onChange={(e) => {
